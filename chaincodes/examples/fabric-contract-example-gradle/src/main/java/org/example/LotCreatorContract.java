...
@Default
public class LotCreatorContract implements ContractInterface {
    public LotCreatorContract() {
    ...
    @Transaction()
    public LotContract readMyAsset(Context ctx, String myAssetId) {
        boolean exists = myAssetExists(ctx,myAssetId);
        if (!exists) {
            throw new RuntimeException("The asset "+myAssetId+" does not exist");
        }

        LotContract newAsset = LotContract.fromJSONString(new String(ctx.getStub().getState(myAssetId),UTF_8));
        return newAsset;
    }
